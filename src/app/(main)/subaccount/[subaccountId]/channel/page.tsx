...
export default async function Channel({params}:Props) {
  ...
  return (
    <main className="flex w-full  mt-[76px] overflow-scroll ml-[20px] flex-col">
  
    {/* <Call linkUrl='/agency' appId={process.env.AGORA_APP_ID!} channelName={user.agencyId+" Channel"}></Call> */}
    {  show && <div><div className='absolute z-50 left-10 top-2'><Button onClick={()=>setShow(false)} className='bg-red-500 hover:bg-red-600'>Fermer</Button></div> <Call /></div>}
